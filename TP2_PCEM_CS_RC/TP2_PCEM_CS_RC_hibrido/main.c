...
int main(int argc, char *argv[]) {
    ...
    int gen_actual, r, j, runs, i, *sol, *mataux, *solaux, *mat, *mat_num_aux, *arr_rand_p1;
    ...
    while (1) {
        ...
        arr_rand_p1 = malloc(sizeof(int) *
                             (EA_param.capacity)); //alocar memoria - array para random para ver se numero ja saiu no random x
        if (!arr_rand_p1) {
            printf("Erro na alocacao de memoria\n");
            exit(1);
        }
        ...
        for (r = 0; r < runs; r++) {
            ...
            trepa_colinas(sol, mat, pop, EA_param, EA_param.capacity, EA_param.numGenes, opcaoGeraVizinho);
